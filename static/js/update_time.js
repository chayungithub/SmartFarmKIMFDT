...
refA.orderByChild("time").limitToLast(1).on("child_added", function(snapshot) {
  ...
  document.getElementById("updated_time_x").innerHTML = "เวลาเปลี่ยนแปลงล่าสุด : "+(unixtime_to_readable(time));
});
...
refA.orderByChild("time").limitToLast(1).on("child_added", function(snapshot) {
	var changedData = snapshot.val();
	var time = changedData.time;
  document.getElementById("updated_time_a").innerHTML = "เวลาเปลี่ยนแปลงล่าสุด : "+(unixtime_to_readable(time));
});
...
refB.orderByChild("time").limitToLast(1).on("child_added", function(snapshot) {
	var changedData = snapshot.val();
	var time = changedData.time;
  document.getElementById("updated_time_b").innerHTML = "เวลาเปลี่ยนแปลงล่าสุด : "+(unixtime_to_readable(time));
});
...
refC.orderByChild("time").limitToLast(1).on("child_added", function(snapshot) {
	var changedData = snapshot.val();
	var time = changedData.time;
  document.getElementById("updated_time_c").innerHTML = "เวลาเปลี่ยนแปลงล่าสุด : "+(unixtime_to_readable(time));
});
